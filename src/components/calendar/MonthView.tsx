        ...
        <div className="hidden sm:block w-12 border-r border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800">
          ...
          {weeks.map((week) => (
            <div
              ...
              className="px-2 py-1.5 text-center text-xs font-medium text-gray-400 dark:text-gray-500 border-b border-gray-100 dark:border-gray-800 flex items-center justify-center min-h-[110px]"
            >
            ...
            {WEEKDAY_LABELS.map((label, i) => (
              <div
                ...
                className={`px-2 py-2 text-center text-xs font-medium ${
                  i === 5 || i === 6 ? 'text-red-400' : 'text-gray-500 dark:text-gray-400'
                }`}
              >
                {label}
              </div>
                  ...
                  {week.map((date) => {
                    ...
                    return (
                      <div
                        ...
                        className={`relative min-h-[110px] border-b border-r border-gray-100 dark:border-gray-800 p-1.5 cursor-pointer transition-colors ${
                          isCurrentMonth ? 'bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800' : 'bg-gray-50 dark:bg-gray-950'
                        } ${isFocused ? 'ring-2 ring-inset ring-blue-400' : ''} ${
                          draggedOverDate === dateStr ? 'bg-blue-50 dark:bg-blue-950 ring-2 ring-inset ring-blue-400' : ''
                        }`}
                      >
                        ...
                        {isExpanded && (
                          <div
                            ...
                            className="absolute left-0 top-full z-30 mt-1 w-56 max-h-72 overflow-auto bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg shadow-lg py-1"
                          >
                            {dayTasks.map((task) => {
                              const bgColor = getTaskBackgroundColor(task)
                              const textColor = getTaskTextColor(task)
