...
### 決策 11：月視圖任務顏色和狀態視覺化
**選擇：** 任務底色基於所屬清單的顏色；過期和已完成的任務顯示灰色；已完成任務加打勾標記

**理由：**
- 利用現有 `List.color` 欄位提供視覺分類
- 灰色 + 打勾能清晰區分任務狀態，無需額外欄位
- 收集箱（無 listId）任務預設藍色，確保所有任務都有視覺識別

**實現：**
```typescript
// 任務底色邏輯
const getTaskBackgroundColor = (task: Task, list?: List): string => {
  if (task.completedAt) return 'bg-gray-200' // 已完成 → 灰色
  if (task.dueDate && isPast(task.dueDate)) return 'bg-gray-200' // 過期 → 灰色
  return list?.color ? `bg-${list.color}-100` : 'bg-blue-100' // 清單顏色或藍色
}

// 顯示邏輯
const showCheckmark = (task: Task): boolean => {
...
### 決策 12：月視圖拖曳互動（Google Calendar 風格）
**選擇：** 支持拖曳任務到新日期以更改截止日期；拖曳時任務浮起且透明；放開時保存

**理由：**
- Google Calendar 是熟悉的拖曳模式，使用者已知悉
- 提供快速重新排程方式，無需打開模式
- 即時視覺反饋提升使用體驗

**互動流程：**
1. 按住任務 → 浮起 + 半透明 (opacity: 0.7)
2. 拖過日期儲存格 → 預覽高亮該日期
3. 放開：
   - **成功** → dueDate 更新，任務移動到新位置
   - **失敗** → 任務回到原位置，顯示錯誤提示

**實現細節：**
- 使用瀏覽器原生 DnD API 或 react-beautiful-dnd
- `updateTask()` Server Action 返回成功/失敗狀態
- 失敗時不更新 UI（保持原位置）
- 禁止拖曳已完成的任務

### 決策 13：月視圖滾動導航
**選擇：** 向上滾動 → 上月；向下滾動 → 下月；焦點日期保持不變（如果存在於新月份）
...
**實現：**
- 監聽月視圖容器的 wheel 事件
- 使用防抖 (debounce) 避免快速多次切換
- 焦點日期邏輯：
  - 如果 focusDate 在新月份 → 保持
  - 如果 focusDate 不在新月份（如 1月31日 → 2月）→ 切換到月末日期

### 決策 14：月視圖週數顯示
**選擇：** 在月視圖左邊每週顯示 ISO 8601 週數
...
<div class="grid grid-cols-8">
  <div class="col-1">週6</div>  <!-- 該週的 ISO 週數 -->
  <div class="col-2-8"><!-- 7天日期儲存格 --></div>
</div>
```
...
- 拖曳時禁用其他互動（按鈕、點擊）
- 保存失敗時立即回滾，避免孤立狀態
- 使用樂觀更新應謹慎，保持伺服器為真實來源
- 測試網路延遲下的拖曳行為

### 風險 7：滾動導航與頁面滾動衝突
**緩解：**
- 滾動事件僅在月視圖容器上監聽，不在全局
- 如果容器不可滾動（適應屏幕高度），月份滾動則生效
- 需要明確的 UX 指導（提示或教程）
